...
unsafe fn flatten_expr(
    ...
    match e {
        ...
        Expr::Op(lhs, op, rhs,) => match op {
            Opcode::Add => {
                let lhs =
                    flatten_expr(context, module, symbols, builder, *lhs,);
                let rhs =
                    flatten_expr(context, module, symbols, builder, *rhs,);
    ...
    for i in stmts {
        match i {
            Either::Left(Stmt::Attr(Variable::Single(v,), e,),) => {
                if let Err(s,) =
                    build_attr(context, module, &my_symbols, builder, v, *e,)
                {
                    panic!(s);
                }
            }
            ...
            Either::Left(Stmt::If(cond, then, else_ifs, None,),) => {
                ...
                let cond =
                    flatten_expr(context, module, &my_symbols, builder, *cond,);
...
pub unsafe fn gen(decls: Vec<Decl,>) {
    ...
    for i in decls {
        match i {
            Decl::Single(n, t, e,) => {
                global_add_variable(context, module, &mut symbols, n, t, e,)
            }
            Decl::Array(n, t, s, e,) => {
                global_add_array(context, module, &mut symbols, n, t, s, e,)
            }
            Decl::Func(n, t, a, b,) => {
                global_add_func(context, module, &mut symbols, n, t, a, b,)
            }
        }
    }
